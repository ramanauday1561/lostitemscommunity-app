  ...
  const redirectTo =
    Platform.OS === 'web'
      ? typeof window !== 'undefined'
        ...
        : undefined
      : AuthSession.makeRedirectUri({ scheme: 'lostitems', path: 'auth/callback' });

  const { data, error } = await supabase.auth.signInWithOAuth({
    provider,
    options: {
      redirectTo,
      // On native we drive the browser ourselves, so ask Supabase for the
      // URL rather than letting it try to navigate.
